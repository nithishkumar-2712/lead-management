...
const handleDelete = async (id, isDeleted) => {
  ...
  const result = await Swal.fire({
    title: isDeleted ? "Activate Role?" : "Deactivate Role?",
    text: isDeleted
  ...
  try {
    ...
    const { data } = await axios.put(`/api/Roledelte/${id}`, {
      isActive: !isDeleted,
    });
...
const handleAdd = async () => {
  ...
  try {
    ...
    const { data } = await axios.post("/api/creatRole", {
...
{UserRole && UserRole.length > 0 ? (
  ...
  <div className="no-data">
    ...
    <p>
      There are no user roles available.
    </p>
